...
static inline bool
is_cpg(const uint32_t x) {return x == 0;}

static inline bool
is_chh(const uint32_t x) {return x == 1;}

static inline bool
is_cxg(const uint32_t x) {return x == 2;}

static inline bool
is_ccg(const uint32_t x) {return x == 3;}
...
static void
update_lc(const bool mut, const uint32_t n_reads, const uint32_t n_meth,
          LevelsCounter &lc) {
  ...
  if (n_reads > 0) {
    ++lc.sites_covered;
    lc.max_depth = std::max(lc.max_depth, static_cast<uint64_t>(n_reads));
    lc.total_c += n_meth;
    lc.total_t += n_reads - n_meth;
    const double meth =
      n_reads == 0 ? 0.0 : n_meth / static_cast<double>(n_reads);
    lc.total_meth += meth;
...
static void
write_output(const bam_header &hdr,
             bgzf_file &out,
             bgzf_file &out_sym,
  ...
  double prev_unconverted = 0.0;
  uint32_t prev_n_reads = 0;
  size_t prev_idx = 0;
  bool prev_is_c = false;
  bool prev_mut = false;
  uint32_t offset = 0;
      ...
      if (is_cpg(the_tag) && !is_c && prev_is_c) {
        ...
        buf << sam_hdr_tid2name(hdr, tid) << '\t' << prev_idx << '\t'
            ...
            << ((n_reads + prev_n_reads) > 0 ?
                (unconverted + prev_unconverted)/(n_reads + prev_n_reads) : 0.0) << '\t'
            << n_reads + prev_n_reads
            << '\n';
        if (!out_sym.write(buf.c_str(), buf.tellp()))
          throw dnmt_error("error writing output");
      }
...
template <class CS>
static void
output_skipped_chromosome(const int32_t tid,
...
process_reads(const bool VERBOSE, const bool show_progress,
              ...
              const string &counts_outfile,
              const string &sym_outfile,
              const string &levels_outfile,
              const string &bsrate_outfile,
              const string &states_outfile) {
  ...
  bgzf_file out(counts_outfile, output_mode);
  if (!out) throw dnmt_error("error opening output file: " + counts_outfile);

  // open the sym output file
  bgzf_file out_sym(sym_outfile, output_mode);
  if (!out_sym) throw dnmt_error("error opening output file: " + sym_outfile);

  quick_buf buf; // keep underlying buffer space?
  buf.clear();

  // open the states output file
  bgzf_file out_states(states_outfile, output_mode);
  if (!out_states) throw dnmt_error("error opening output file: " + states_outfile);
  ...
  while (hts.read(hdr, aln)) {
    if (get_tid(aln) == prev_tid) {
      ...
      for (auto i = prev_tid + 1; i < tid; ++i)
        output_skipped_chromosome(i, tid_to_idx, hdr, chroms_beg,
                                  chrom_sizes, counts, out, out_sym);
    ...
    if (use_this_chrom) {
      ...
      if (bam_is_rev(aln))
        count_states_neg(INCLUDE_CPGS, *chrom_itr,
                         aln, summaries, hanging);
      else
        count_states_pos(INCLUDE_CPGS, *chrom_itr,
                         aln, summaries, hanging);
    }
    ...
    if (has_cpgs) {
      ...
      buf << sam_hdr_tid2name(hdr, aln) << '\t' << first_cpg_index << '\t'
          << seq << '\n';
      if (!out_states.write(buf.c_str(), buf.tellp())) {
  ...
  for (auto i = prev_tid + 1; i < hdr.h->n_targets; ++i)
    output_skipped_chromosome(i, tid_to_idx, hdr,
                              chroms_beg, chrom_sizes, counts, out, out_sym);
  ...
  if (!out_levels) throw std::runtime_error{"bad output file: " + levels_outfile};
...
main_summary(int argc, const char **argv) {
  ...
  try {
    ...
    string chroms_file;
    string levels_outfile;
    string bsrate_outfile;
    string counts_outfile;
    string sym_outfile;
    string states_outfile;
    int n_threads = 1;
    ...
    opt_parse.add_opt("threads", 't', "threads to use (few needed)",
                      false, n_threads);
    opt_parse.add_opt("levels-out", '\0', "levels output file",
                      true, levels_outfile);
    opt_parse.add_opt("bsrate-out", '\0', "bsrate output file",
                      true, bsrate_outfile);
    opt_parse.add_opt("counts-out", '\0', "counts output file",
                      true, counts_outfile);
    opt_parse.add_opt("sym-out", '\0', "sym output file",
                      true, sym_outfile);
    opt_parse.add_opt("states-out", '\0', "states output file",
                      true, states_outfile);
    ...
    if (VERBOSE)
      cerr << "[input BAM/SAM file: " << mapped_reads_file << "]" << endl
           << "[counts file: " << counts_outfile << "]" << endl
           << "[sym file:    " << sym_outfile << "]" << endl
           << "[bsrate file: " << bsrate_outfile << "]" << endl
           << "[levels file: " << levels_outfile << "]" << endl
           << "[states file: " << states_outfile << "]" << endl
